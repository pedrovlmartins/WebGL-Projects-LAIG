...
class CircularAnimation extends Animation {
     ...
     */
    constructor(scene, id, speed, x, y, z, radius, initialAngle, rotationAngle, factor) {
        super(scene, id);
    ...
    animate(t) {
		...
		*/
		...
		this.currentAngle = this.initialAngle + this.speed * this.movementTime;
		...
		if (this.currentAngle * radToDegree < this.initialAngle + this.rotationAngle) {
			this.scene.translate(this.center[0], this.center[1], this.center[2]); // translates to the center first (the center of the object)
			this.scene.rotate(this.factor * this.currentAngle, 0, 0, 1); // rotates to the new angle (ensures helicopter type movement)
			...
			this.scene.rotate(this.factor * (this.initialAngle + this.rotationAngle) * Math.PI / 180, 0, 0, 1);
			this.scene.translate(this.radius, 0, 0);
			this.ended = true;
			this.scene.changingPlayer = false;
			
			if (this.speed != 10000000 && !this.countdownset) {
				this.scene.setCountdown();
